        ...
        case 'procesoshome':
            ...
            $('.new-proceso').mouseover(function (e) {
                ...
                if ($(window).width() > 600) {
                    ...
                    var currimaHover = $(e.currentTarget).find('.hoverblack');
                    TweenMax.to($(currima), .2, {
                        alpha: 0, onComplete: function () {
                            var id = $(currimaHover).attr('src');
                            $(currima).attr('src', id);
            ...
            $('.new-proceso').mouseout(function (e) {
                if ($(window).width() > 600) {
                    ...
                    TweenMax.to($(currima), .2, {
                        alpha: 0, onComplete: function () {
                            var imageNoOver = 'ImageResolver.ashx?Id=' + currima.attr('data-imageNoHover');
                            $(currima).attr('src', imageNoOver);
...
function loadProjectImagesJson($section) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var node = search($section, imagesProjectJson);
            preloadProjectImges(node.ImageUrls);
        },
        error: function (e) {
